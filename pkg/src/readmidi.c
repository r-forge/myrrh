...
static MidiEventList *read_midi_event(MidiSong *song)
{
    ...
    for (;;)
    {
        ...
        {
            SNDDBG(("Error while trying to read data from file\n"));
            return 0;
        }
...
static MidiEvent *groom_list(MidiSong *song, Sint32 divisions,Sint32 *eventsp,
                             Sint32 *samplesp, int convertTimeToSamples)
{
...
MidiEvent *read_midi_file(MidiSong *song, Sint32 *count, Sint32 *sp, int convertTimeToSamples)
{
    ...
    if (divisions_tmp<0)
    {
        ...
        divisions=(Sint32)(divisions_tmp);

    if (len > 6)
    {
        SNDDBG(("MIDI file header size %u bytes\n", len));
    ...
    return groom_list(song, divisions, count, sp, convertTimeToSamples);
}
